...
def create_args_string(num):
    L = [] # L = ['?' for i in range(num)]
    for n in range(num):
        L.append('?')
    return ','.join(L)  #','.join(['?' for i in range(7)])
...
class IntegerField(Field):
    def __init__(self, name=None, is_primary_key=False, default=0, ddl = 'bigint'):
        super().__init__(name, ddl, is_primary_key, default)
...
class ModelMetaClass(type):
    def __new__(cls,name, bases, attrs):
        print('run:%s %s %s() with Class name %s' % (cls.__module__, cls.__name__,cls.__new__.__name__,name))
        if name == 'Model':
        ...
        attrs['__mapping__'] = mappings # attrs <--> db.column => '__mapping__' 保存属性和列的映射关系
        attrs['__table__'] = tableName
        attrs['__primary_key__'] = primaryKey # 主键属性名
        attrs['__fields__'] = fields #除主键外的属性名list
